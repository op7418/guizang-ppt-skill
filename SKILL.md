...
description: 生成横向翻页网页 PPT（单 HTML 文件），含 WebGL 背景、章节幕封、数据大字报、图片网格等模板。提供两种风格：① "电子杂志 × 电子墨水"（衬线 + 流体背景 + 暖色） ② "瑞士国际主义"（无衬线 + 网格点阵 + IKB/柠檬黄/柠檬绿/安全橙高亮）。当用户需要制作分享 / 演讲 / 发布会风格的网页 PPT，或提到"杂志风 PPT"、"瑞士风 PPT"、"Swiss Style"、"horizontal swipe deck"、"公众号封面"、"小红书封面"、"分享卡"时使用。
---
...
用户提到"公众号封面 / 21:9 头图 / 分享卡 / 小红书封面 / 视频号封面 / 基于这份 PPT 做封面"时,不要只按普通 PPT 页面处理,而是切换到封面工作流。

先问或自行判断 3 件事:

- **平台与比例**:公众号头图 `21:9`;公众号分享卡 `1:1`;小红书 `3:4`;视频号横版 `16:9`;如果用户未说明,默认先做公众号 `21:9`。
- **信息层级**:主标题、短副标题、署名/日期/品牌是否需要出现;封面文字必须比 PPT 内页更少。
- **风格继承**:如果来自已有 deck,沿用 deck 的 Style A / Style B、主题色和关键词;如果单独生成,按内容推荐 A 或 B。

封面生成原则:

- 封面不是一页 PPT 截图。它要在信息流里一眼成立:大标题短、主体明确、留白足、缩略图可读。
- 公众号 `21:9` 的核心标题和主体放在中央 70% 安全区,两侧留呼吸空间,避免移动端裁切。
- 小红书 `3:4` 更适合单一强主体 + 2-4 个短标签,不要塞长段落。
- 分享卡 `1:1` 要减少横向结构,优先居中构图或上下分区。
- 如果需要生成图片,先读 `references/image-prompts.md`;如果是截图封面,先读 `references/screenshot-framing.md`。
- 输出文件放到 `images/covers/`,命名遵守 `{platform}-{ratio}-{slug}.{ext}`;如果生成 HTML 封面,放到 `covers/`。
