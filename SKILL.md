...
| 用户需要 GPT-M 2.0 生成截图再设计 / 信息图 / 证据墙 | B 也很合适(S15/S16/S22 是瑞士风图片常用版式) |
...
**默认不要让图片 `align-self:end`**——会滑到页面底部,很容易碰到分页组件。用 grid 容器 + `align-items:start`(template 已预设)让图片贴顶即可;如需贴近底部,只能使用模板已内置安全区的登记版式/组件,不要手写 `bottom:2vh`。
...
22. **图片同组一致**——S15/S16 多图格必须统一比例、高度、边距、线条粗细;信息图/UI 图按来源决定是否加 `.fit-contain`
23. **组件角色要正确**——S22 的 KPI/说明是必选;S15/S16 图片格需要 caption 或信息锚点;数据专用版式必须有真实数据,不能靠文案硬填
